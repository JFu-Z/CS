...
parser.add_argument("--dataset_name", type=str, default="dataset/datas",
    ...
    # print("\nsave my model finished !!")
       ## 每间隔几个epoch保存一次模型
        if epoch>190:
            ...
            torch.save(D_A.state_dict(), "save/%s/D_A_%d.pth" % (opt.dataset_name, epoch))
            torch.save(D_B.state_dict(), "save/%s/D_B_%d.pth" % (opt.dataset_name, epoch))
    open('g_MPCSA','w',encoding='utf-8').write(json.dumps(g_loss))
    open('d_MPCSA','w',encoding='utf-8').write(json.dumps(d_loss))

def test():
    ## 超参数设置
    parser = argparse.ArgumentParser()
    parser.add_argument('--batchSize', type=int, default=1, help='size of the batches')
    parser.add_argument('--dataroot', type=str, default='dataset/test', help='root directory of the dataset')
    ...
    parser.add_argument('--height', type=int, default=384, help='size of the data (squared assumed)')
    parser.add_argument('--width', type=int, default=512, help='size of the data (squared assumed)')
    parser.add_argument('--cuda', type=bool, default=True, help='use GPU computation')
    parser.add_argument('--n_cpu', type=int, default=8, help='number of cpu threads to use during batch generation')
    parser.add_argument('--generator_A2B', type=str, default='save/dataset/main/G_AB_198.pth',
                        help='A2B generator checkpoint file')
    parser.add_argument('--generator_B2A', type=str, default='save/dataset/main/G_BA_198.pth',
    ...
    input_A = Tensor(opt.batchSize, opt.channels, opt.height, opt.width)
    input_B = Tensor(opt.batchSize, opt.channels, opt.height, opt.width)
    '''构建测试数据集'''
    ...
    for i, batch in enumerate(dataloader):
        ...
        ## 保存图片
        save_image(fake_A, 'output/A/%02dA.png' % (i + 1))
        save_image(fake_B, 'output/B/%02dB.png' % (i + 1))
        print('processing (%02d)-th image...' % (i))
    end_time = time.time()
    execution_time = end_time - start_time
    print(f"测试耗时:{execution_time}秒")
    dummy_input = torch.randn(1, 3, 256, 256).to('cuda')
    flops, params = profile(netG_A2B, (dummy_input,))
    print('flops: %.4fG, params: %.4fM' % (flops / 1000000000.0, params / 1000000.0))
